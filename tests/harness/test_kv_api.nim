import std/[os, strutils, options, base64]
import chronos
import curly
...
proc testKvApiPutGetDelete(kvUrl: string) {.async.} =
  ...
  doAssert decode(getResp.body) == encrypted, "GET body doesn't match PUT data"
...
proc testKvApiOverwrite(kvUrl: string) {.async.} =
  ...
  doAssert decode(getResp.body) == enc2, "GET after overwrite doesn't return latest data"
